...
export const vaultIndex: VaultSearchDocument[] = [
  ...
  ...seasons.map(season => ({ id: `season:${season.year}`, type: "season" as const, title: `${season.year} ${season.location} Raiders`, subtitle: `${season.record} · ${season.coach}`, text: `${season.year} ${season.location} Raiders ${season.record} ${season.coach} ${season.finish} ${season.note}`, href: "/seasons", year: season.year })),
  ...
  ...moments.map(moment => ({ id: `moment:${moment.slug}`, type: "moment" as const, title: moment.title, subtitle: `${moment.date}${moment.opponent ? ` · vs. ${moment.opponent}` : ""}`, text: `${moment.title} ${moment.date} ${moment.opponent ?? ""} ${moment.summary} ${moment.tags.join(" ")}`, href: "/moments", year: Number(moment.date.slice(0, 4)) })),
