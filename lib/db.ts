  ...
  readyPromise = (async () => {
    try {
      ...
      if (!check[0]?.entities || currentVersion !== EXPECTED_DB_VERSION) {
        ...
        const { stdout, stderr } = await execFileAsync(process.execPath, ["scripts/migrate-v16-modern-draft.mjs"], { cwd: process.cwd(), env: process.env, timeout: 90000 });
