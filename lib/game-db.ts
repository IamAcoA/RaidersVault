...
export async function getGameExhibit(slug: string): Promise<GameExhibit | null> {
  ...
  if (!databaseConfigured() || !(await ensureDatabaseReady())) return fallbackRow ? { ...fallbackRow, related: [], database: false } : null;
  try {
    const sql = db();
    ...
    `;
    const row = rows[0];
    if (!row) return fallbackRow ? { ...fallbackRow, related: [], database: false } : null;
    ...
    return {
      ...
      related: relatedRows.map(item => ({
        ...
        relation: item.relation_type === "game_of_season" ? "Season" : "Championship record",
        href: item.entity_type === "championship" ? `/championships/${item.slug}` : "/seasons"
      })),
      database: true
    };
  } catch {
    return fallbackRow ? { ...fallbackRow, related: [], database: false } : null;
  }
}
